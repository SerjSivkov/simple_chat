defmodule SimpleChat do
  ...
  def loop() do
    receive do
      {:new_chat, name} -> ChatService.add(name)
      {:new_user, login} -> UserService.add(login)
      {:send_message_to_chat, message, chat_id, user_login} ->
        ChatService.send_message_to_users(message, chat_id, user_login)
      {:join_chat, user_login, chat_id} -> ChatModel.join_chat(user_login, chat_id)
      {:leave_chat, user_login, chat_id} -> ChatModel.leave_chat(user_login, chat_id)
    end
