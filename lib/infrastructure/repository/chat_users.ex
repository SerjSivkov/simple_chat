defmodule SimpleChat.Infrastructure.Repository.ChatUsers do
  ...
  @spec add_user(binary(), User.user()) :: :ok | {:error, :not_found}
  def add_user(chat_id, %User{} = user) do
    with {:ok, _chat} <- Chat.get(chat_id),
         true <- :ets.lookup(@table, chat_id)
                 |> add_user_to_chat(chat_id, user)
    do
      :ok
    end
  end

  @spec remove_user(binary(), User.user()) :: true
  def remove_user(chat_id, %User{} = user), do
    case get(chat_id) do
      {:ok, users} ->
        new_users =
  ...
  @spec get(binary()) :: {:error, :not_found} | {:ok, [User.user(), ...]}
  def get(chart_id) do
